...
<div class="page clearfix">
	...
	<form class="form-horizontal form form-validate" id="form1" action="" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">更新服务到期时间</label>
			<div class="col-md-6">
				<p class="form-control-static">
					<?php  if(is_array($cache) && !empty($cache['endtime'])) { ?>
						<?php  if($cache['endtime'] > TIMESTAMP) { ?>
							<span class="label label-success"><?php  echo date('Y-m-d', $cache['addtime'])?> ~ <?php  echo date('Y-m-d', $cache['endtime'])?></span>
							<?php  if($cache['endtime'] < TIMESTAMP + 864000) { ?>
								<a href="<?php  echo $url;?>" target="_blank" class="btn btn-warning">更新服务即将到期,点击续费</a>
							<?php  } ?>
						<?php  } else { ?>
							<span class="label label-danger"><?php  echo date('Y-m-d', $cache['addtime'])?> ~ <?php  echo date('Y-m-d', $cache['endtime'])?></span>
							<br/><br/>
							<a href="<?php  echo $url;?>" target="_blank" class="btn btn-danger">更新服务已到期,点击续费</a>
						<?php  } ?>
					<?php  } else { ?>
						<span class="label label-danger">未知,点击下面的“验证授权”进行查询</span>
					<?php  } ?>
				</p>
			</div>
		</div>
			...
			<div class="col-md-6">
				<p class="form-control-static">
					<span class="label label-info">
						<?php  echo $familys[MODULE_FAMILY]['title'];?>-<?php echo MODULE_VERSION;?>-<?php echo MODULE_RELEASE_DATE;?>
					</span>
				</p>
			</div>
		</div>
