...
class Order extends Backend{
    ...
    public function detail($ids = NULL)
    {
        ...
        //empty($result['end_img']) || $result['end_img'] = '/addons/make_speed/core/public/' . $result['end_img'];
    ...
    public function tasking($ids = NULL){
        if ($this->request->isPost()) {
            ...
            $this->success('分配成功! 已发送短信通知技师');
        }

        $id = intval($ids);
        //获取订单范围骑手
        $riderPoint = $this->getScopeTechnician($id);
        $rider_id = '';
        ...
        $rider = Db::name('rider')
            ...
            ->join('homemaking_technician f','r.id = f.rider_id')
            ->where([ 'f.status'=> 2])
            ->column('r.id,r.real_name,r.mobile,r.avatar,f.category_id');
        ...
        foreach ($rider as $k => $v){
            ...
            if(isset($v['category_id'])){
                $vehicle = Db::name('homemaking_category')->where(['id'=>['in',$v['category_id'] ] ])->column('title');
                $rider[$k]['category_id'] = implode(',',$vehicle);
            }
        }
    ...
    public function getScopeTechnician($order_id, $type=0){
        ...
        $distance = Db::name('setting')->where(array('key'=>'rider_distance','uniacid'=>$GLOBALS['uniacid']))->find();
        ...
        $rider = Db::name('rider_info')
            ->alias('i')
            ->join('homemaking_technician d','i.rider_id = d.rider_id')
            ->where($where)
