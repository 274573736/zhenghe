...
class Orderlist extends Backend
{
    ...
    public function index()
    {
        ...
        //搜索字段
        $this->searchFields   = 'order_code,business.name';
        ...
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model
                ->with(['user','business'=>function($query){
                     if(!empty($GLOBALS['city_id'])){
                         $query->where(['business.city_id'=>$GLOBALS['city_id']]);
                     }
                }])
                ->where($where)
                ->where($wheres)
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                try {
                    ...
                    //取消待付,需退款
                    if($params['status']<2 && $order['status']>=2){
    ...
    public function tasking($ids = NULL){
        if ($this->request->isPost()) {
            ...
            $order = Db::name('order')->where(array('id'=>$oid))->field('order_code,business_id,role,status')->find();
            ...
            $business = Db::name('business')->where(array('id'=>$order['business_id']))->find();
            //饿了么订单状态更新
            if($order['role']=='eleme'){
                if(!empty($business) && !empty($business['token'])) {
                    $re = update_eleme_order($order['order_code'], $business,$mobile, 'DELIVERY_KNIGHT_ACCEPT');
                    if($re!==true){
                        $this->error($re);
                    }
                }
            ...
            //更新位置
            update_eleme_location($order['order_code'], $business,$rider['lat'],$rider['lng']);
