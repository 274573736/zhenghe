...
class Backend extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if(!extension_loaded('openssl')){
            $this->error('openssl扩展尚未开启','');
        }

        $url = 'http://sq.7make.com/api/v1/auth';
        $re = json_decode(file_get_contents($url),true);
        if( $re['code'] !== 200 ){
          //  $this->error($re['msg'],'https://'.$_SERVER['HTTP_HOST']);
        }
        ...
        $GLOBALS['uniacid'] = !empty($admin['uniacid']) ? intval($admin['uniacid']) : (int)$_W['uniacid'];
        ...
        if (!$this->auth->match($this->noNeedLogin)) {
            ...
            if (!$this->auth->isLogin()) {
                ...
                $this->redirect(tp_url('index/login') );
            }
            // 判断是否需要验证权限
        ...
        //所属城市ID
        $GLOBALS['city_id'] = !empty($admin['city_id']) ? intval($admin['city_id']) : 0;


        $admin['avatar'] = $domain.'/attachment/headimg_'.$GLOBALS['uniacid'].'.jpg?time='.time();
